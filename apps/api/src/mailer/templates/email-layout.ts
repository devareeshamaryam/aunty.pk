...
export function renderInfoCards(
  shippingAddress:
    | { street?: string; area?: string; city?: string; state?: string; zipCode?: string; phone?: string }
    | undefined,
  paymentMethod: string,
  orderId: string,
): string {
  const addr = shippingAddress || {};
  const line1 = [addr.street, addr.area].filter(Boolean).join(', ');
    ...
    <div class="info-grid">
      <div class="info-card">
        ...
        <div class="info-card-text">
          ${line1 || 'Map location provided'}<br/>
          ${line2}<br/>
