...
export class ProductsService {
  ...
  async findOne(id: string) {
    const product = await this.productModel
      .findById(id)
      .populate('category')
      .populate('relatedProducts', 'name slug price images variants isFeatured')
      .lean();
  ...
  async update(id: string, dto: UpdateProductDto) {
    ...
    // Drop empty / null slug from frontend so we never overwrite the existing required field
    if (!dto.slug || (typeof dto.slug === 'string' && dto.slug.trim() === '')) {
      delete dto.slug;
    }
    // Drop empty / null name as well (also required)
